...
function ConfirmModal({ open, title, message, onYes, onNo, darkTheme = false }) {
  if (!open) return null;
  const modalStyle = darkTheme ? { background: 'rgba(30,41,59,0.95)', color:'#f8fafc' } : { background: '#fff', color: '#000' };
  return (
    <div style={{
      ...
      <div style={{ width: 360, maxWidth: "94%", borderRadius: 12, padding: 18, ...modalStyle }}>
        <h3 style={{ margin: 0, marginBottom: 8 }}>{title}</h3>
        <p style={{ marginTop: 0, marginBottom: 16 }}>{message}</p>
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
          <button onClick={onNo} style={{ padding: "8px 12px", background: "#f3f4f6", borderRadius: 8, border: "none", cursor: "pointer" }}>Cancel</button>
          <button onClick={onYes} style={{ padding: "8px 12px", background: "linear-gradient(90deg,#8b5cf6,#06b6d4)", color: "#fff", borderRadius: 8, border: "none", cursor: "pointer" }}>Add Anyway</button>
        </div>
      </div>
    </div>
  );
...
function PlannerCell({ 
  isoDate, mealTime, meals = [], onDropMeal, onSlotClick, onRemove, highlightedRecipe,
  plannerMode=false, minimized=false, darkTheme=false
}) {
  ...
  const mergedMeals = useMemo(() => {
    ...
    meals.forEach((m, rawIndex) => {
      ...
      if (prev) {
        map.set(id, { ...prev, servings: (prev.servings ?? 0) + (m.servings ?? 1), __sourceIndices: [...prev.__sourceIndices, rawIndex] });
      } else {
        map.set(id, { ...m, servings: m.servings ?? 1, __sourceIndices: [rawIndex] });
      }
    });
  ...
  return (
    <td ref={dropRef} style={cellStyle} onClick={() => highlight && onSlotClick && onSlotClick(isoDate, mealTime)}>
...
export default function WeekPlanner({
  ...
  const cancelPending = useCallback(() => { setPending(null); setConfirmOpen(false); }, []);
  ...
  const handleDropMeal = useCallback(async (recipe, isoDate, mealTime) => {
    ...
    if (existingIdx !== -1) {
      ...
      try { await removeMeal({ key: targetKey, index: existingIdx, serverId: existing.serverId ?? null }); } catch {}
      if (sourceKey && sourceKey !== targetKey) {
        const matchIdx = srcArr.findIndex(it => (it.clientId && recipe.clientId && it.clientId === recipe.clientId) || (it.serverId && recipe.serverId && it.serverId === recipe.serverId));
        if (matchIdx !== -1) await removeMeal({ key: sourceKey, index: matchIdx, serverId: srcArr[matchIdx].serverId ?? null });
      }
  ...
  const containerStyle = darkTheme ? { padding: 16, color: '#f8fafc' } : { padding: 16 };

  return (
    <div style={containerStyle}>
      <ConfirmModal open={confirmOpen} title="Allergen Warning" message="This recipe contains one or more allergens you marked. Are you sure you want to add it?" onYes={confirmAndAdd} onNo={cancelPending} darkTheme={darkTheme} />

      {/* week navigation */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <button onClick={() => setWeekOffset(o => o - 1)} style={{ padding: "6px 10px", borderRadius: 8, cursor: "pointer" }}>Previous</button>
          <button onClick={() => setWeekOffset(0)} style={{ padding: "6px 10px", borderRadius: 8, cursor: "pointer" }}>This Week</button>
          <button onClick={() => setWeekOffset(o => o + 1)} style={{ padding: "6px 10px", borderRadius: 8, cursor: "pointer" }}>Next</button>
        </div>
        {/* Refresh button removed - auto-refresh happens via useEffect */}
      </div>

      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr>
            <th style={{ textAlign: "left", width: 120, padding: 8 }}>Meal Time</th>
            {weekDates.map(({ day, displayDate, isoDate }, idx) => {
              const isToday = isoDate === todayLocal;
              return (
                <th key={`${day}-${idx}`} style={{ padding: 8, textAlign: "center", background: isToday ? "rgba(99,102,241,0.12)" : "transparent", borderRadius: 6 }}>
                  <div style={{ fontSize: 12, color: "#6b7280" }}>{displayDate}</div>
  
                  <div style={{ fontSize: 12, color: "#94a3b8" }}>{isoDate}</div>
                </th>
              );
            })}
          </tr>
        ...
        <tbody>
          {mealTimes.map((mealTime) => (
            <tr key={mealTime}>
              <td style={{ fontWeight: 600, color: "#7c3aed", textAlign: "center", background: "rgba(139,92,246,0.04)", padding: 8 }}>{mealTime}</td>
